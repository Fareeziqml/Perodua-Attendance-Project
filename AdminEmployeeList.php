...
if (isset($_POST['add'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO employee (employee_id, name, email, department, role, photo, password) VALUES (?,?,?,?,?,?,?)");
    $stmt->bind_param("sssssss", $emp_id, $name, $email, $department, $role_field, $photo, $password);
...
if (isset($_POST['edit'])) {
    ...
    $sql = "UPDATE employee SET name=?, email=?, department=?, role=?";
    $params = [$name, $email, $department, $role_field];
    $types = "ssss";
...
.card-gm h3, .card-gm h2,
.card-emp h3, .card-emp h2 {
    color: #111;  
}
        ...
        <tr>
            ...
            <td>
                <button class="btn btn-edit" onclick="openEditForm('<?= $row['employee_id'] ?>','<?= htmlspecialchars($row['name']) ?>',
                                                                    '<?= htmlspecialchars($row['email']) ?>','<?= htmlspecialchars($row['department']) ?>',
                                                                    '<?= htmlspecialchars($row['role']) ?>')">Edit Profile</button>
...
<div class="form-popup" id="editForm">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <input type="email" name="email" id="edit_email" placeholder="Insert Email" required>
...
function openEditForm(id,name,email,dept,role){ 
...
// Charts
const gmChart = new Chart(document.getElementById('gmChart'), { type:'doughnut', data:{ labels:['GM'], datasets:[{ data:[<?= $total_gm ?>], backgroundColor:['#fc0909ff'] }] }, options:{ responsive:true, plugins:{legend:{display:false}}, cutout:'70%' } });
const empChart = new Chart(document.getElementById('empChart'), { type:'doughnut', data:{ labels:['Employee'], datasets:[{ data:[<?= $total_employee ?>], backgroundColor:['#09f11dff'] }] }, options:{ responsive:true, plugins:{legend:{display:false}}, cutout:'70%' } });
</script>
